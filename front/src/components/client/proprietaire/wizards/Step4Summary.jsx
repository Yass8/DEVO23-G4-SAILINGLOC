    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          ...
          <div>
            <p><span className="font-medium">Moteur :</span> {data.engine_type || "Non renseigné"}</p>
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          ...
          <p><span className="font-medium">Tirant d'eau :</span> {data.water_draft ? `${data.water_draft} m` : "Non renseigné"}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <p className="whitespace-pre-line text-sm">{data.description || "Aucune description"}</p>
      </div>
      ...
      <div>
        ...
        {data.equipment.length > 0 ? (
          ...
          <p className="text-sm">Aucun équipement ajouté</p>
        )}
      </div>
          ...
          <>
            <ul className="list-disc list-inside text-sm">
              {data.photos.map((file, idx) => (
                <li key={idx}>
                  {file.name}
                  {idx === mainIndex && <span className="text-mocha font-semibold ml-1">(principale)</span>}
                </li>
              ))}
            </ul>
          </>
        ) : (
          <p className="text-sm">Aucune photo téléchargée</p>
        )}
      </div>
    </div>
  );
